...
int main(void)
{
  ...
  // thrust::host_vector<int> h_vec_2(N,1);
  // std::generate(h_vec_2.begin(), h_vec_2.end(), 1);
  ...
  // new_end = thrust::reduce_by_key(key_in.begin(), key_in.end(), uni_vect.begin(), key_out.begin(), num_in_bins.begin());
  ...
  thrust::copy(num_in_bins.begin(), num_in_bins.end(), hvalue.begin());
