  ...
  tbb::tick_count mainStartTime = tbb::tick_count::now();
	...
	vector<vector<string>> data; 
	  ...
	  */
	  ...
	  std::cout << "HOOK1" << std::endl;	  
	  cudaSetDevice(0);
	  thrust::sort(thrust::cuda::par.on(stream[0]), key_in_1.begin(), key_in_1.end());
	  // thrust::sort(thrust::cuda::par(stream[0]), key_in_1.begin(), key_in_1.end());
      	  // cudaStreamSynchronize(stream[0]);
	  cudaStreamSynchronize(stream[0]);

	  std::cout << "HOOK2" << std::endl;	  
	  cudaSetDevice(1);
	  thrust::sort(thrust::cuda::par.on(stream[1]), key_in_2.begin(), key_in_2.end());
	  // thrust::sort(thrust::cuda::par(stream[1]), key_in_2.begin(), key_in_2.end());
	  std::cout << "HOOK2.1" << std::endl;	  
	  cudaStreamSynchronize(stream[1]);


	  /*
	  std::cout << "HOOK3" << std::endl;	  
	  for (int i = 0; i < 2; i++)
	  ...
	  */
	  ...
	  /*
          thrust::device_vector<long> dkey_out_3(N,0);
	  thrust::device_vector<long> dvalue_out_3(N,0);

	  for(long i=0; i <new_size_1; i++)
	    {
		dkey_out_3.push_back(dkey_out_1[i]);
		dvalue_out_3.push_back(dvalue_out_1[i]);
	    }

	  for(long i=0; i <new_size_2; i++)
	    {
		dkey_out_3.push_back(dkey_out_2[i]);
		dvalue_out_3.push_back(dvalue_out_2[i]);
	    }
