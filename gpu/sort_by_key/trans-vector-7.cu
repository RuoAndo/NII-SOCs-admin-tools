...
int main( int argc, char* argv[] ) {
  ...
  unsigned int destinationPort_integer;
	...
	try {
	 ...
	 // ofstream outputfile("tmp", std::ios::in | std::ios::ate);  
	 // outputfile.open("tmp",ios::trunc);
	  ...
	  for (unsigned int row = 0; row < data.size(); row++) {
	    ...
	    */
	    ...
	    outputfile << atoi(rec[4].c_str()) << "," << atoi(rec[5].c_str()) << sourceIP_integer << "," << sourceIP_string << "," << sourcePort_integer << "," << destinationIP_integer << "," << destinationIP_string << "," << destinationPort_integer << endl;
