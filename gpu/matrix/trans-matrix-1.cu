...
int main( int argc, char* argv[] ) {
  ...
  unsigned int sourceIP_integer;
  unsigned int sourcePort_integer;
  unsigned int destinationIP_integer;
  unsigned int destinationPort_integer;
	...
	try {
	 ...
	 ofstream outputfile("tmp");
	  ...
	  for (unsigned int row = 0; row < data.size(); row++) {
	    ...
	    std::string srcPort = rec[5];
	    std::string destPort = rec[8];
	    ...
	    std::string p = rec[17];
	    std::string pSent = rec[18];
	    std::string pRecv = rec[19];

	    std::string b = rec[20];
	    std::string bSent = rec[21];
	    std::string bRecv = rec[22];

	    for(size_t c = p.find_first_of("\""); c != string::npos; c = c = p.find_first_of("\"")){
	      p.erase(c,1);
	    }
	    for(size_t c = pSent.find_first_of("\""); c != string::npos; c = c = pSent.find_first_of("\"")){
	      pSent.erase(c,1);
	    }
   	    for(size_t c = pRecv.find_first_of("\""); c != string::npos; c = c = pRecv.find_first_of("\"")){
	      pRecv.erase(c,1);
	    }
	    
	    for(size_t c = b.find_first_of("\""); c != string::npos; c = c = b.find_first_of("\"")){
	      b.erase(c,1);
	    }
	    for(size_t c = bSent.find_first_of("\""); c != string::npos; c = c = bSent.find_first_of("\"")){
	      bSent.erase(c,1);
	    }
	    for(size_t c = bRecv.find_first_of("\""); c != string::npos; c = c = bRecv.find_first_of("\"")){
	      bRecv.erase(c,1);
	    }
	    ...
	    sourceIP_integer = sourceIP_final.to_ullong();
	    ...
	    */
	    ...
	    sourcePort_integer = sourcePort_final.to_ullong();
	    ...
	    destinationIP_integer = destinationIP_final.to_ullong();
	    ...
	    destinationPort_integer = destinationPort_final.to_ullong();
	    ...
	    inaddr = { htonl(destinationIP_integer) };
	    some_addr = inet_ntoa(inaddr);
	    string destinationIP_string = string(some_addr);

	    outputfile << timestamp << "," << sourceIP_integer << ",(" << sourceIP_string << ")," << sourcePort_integer << "," << destinationIP_integer << ",(" << destinationIP_string << ",)" << destinationPort_integer << "," << p << "," << pSent << "," << pRecv << "," << b << "," << bSent << "," << bRecv << endl;
