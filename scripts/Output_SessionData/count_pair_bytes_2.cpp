...
static void CountOccurrences(int nthreads, int N) {
    ...
    parallel_for( blocked_range<MyString*>( Data, Data+N, N ), Tally_sent(table3) );
    tick_count t5 = tick_count::now();

    StringTable table4;    
    tick_count t6 = tick_count::now();
    parallel_for( blocked_range<MyString*>( Data, Data+N, N ), Tally_recv(table4) );
  ...
  int N = atoi(argv[2]);  
	...
	try {
	  ...
	  for (unsigned int row = 0; row < data.size(); row++) {
	    ...
	    string bytes_local = rec[20].c_str();	    
	    bytes_local.pop_back(); 
	    bytes_local.erase(bytes_local.begin() + 0);

	    string bytes_sent = rec[21].c_str();
	    bytes_sent.pop_back(); 
	    bytes_sent.erase(bytes_sent.begin() + 0);
	    
	    string bytes_recv = rec[22].c_str();
	    bytes_recv.pop_back(); 
	    bytes_recv.erase(bytes_recv.begin() + 0);
	    
	    // printf("test \n");
	    // std::cout << bytes_local << std::endl;

	    v_bytes.push_back(std::atoi(bytes_local.c_str()));
	    v_sent.push_back(std::atoi(bytes_sent.c_str()));
	    v_recv.push_back(std::atoi(bytes_recv.c_str()));
