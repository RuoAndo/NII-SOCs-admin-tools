...
int traverse_file(char* filename, int thread_id) {
  ...
  try {
    ...
    const string file_rendered_outward = full_path.string() + "/" + list_string.back() + "_egress"; 
    ofstream outputfile_outward(file_rendered_outward);
    
    const string file_rendered_inward = full_path.string() + "/" + list_string.back() + "_ingress"; 
