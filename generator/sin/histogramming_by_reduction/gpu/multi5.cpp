...
typedef struct _tPair {
  map<unsigned long long, long> cPair;
  pthread_mutex_t mutex;    
} tPair_t;
tPair_t tPair;
...
int traverse_file(char* filename, int thread_id) {
    ...
    */
    ...
    pthread_mutex_lock(&tPair.mutex);
        for(int i = 0; i < new_size; i++)
	  {
	    tPair.cPair.insert(std::make_pair(key_out[i], value_out[i]));
	  }
    pthread_mutex_unlock(&tPair.mutex);
...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    for(auto itr = tPair.cPair.begin(); itr != tPair.cPair.end(); ++itr) {
      std::string timestamp = to_string(itr->first);

      cout << timestamp.substr(0,4) << "-" << timestamp.substr(4,2) << "-" << timestamp.substr(6,2) << " "
		 << timestamp.substr(8,2) << ":" << timestamp.substr(10,2) << ":" << timestamp.substr(12,2)
		 << "." << timestamp.substr(14,3) << ","
		 << itr->second << endl; 
    }
