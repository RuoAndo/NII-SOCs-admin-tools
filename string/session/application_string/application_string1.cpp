...
std::vector<string> v_application_string;
...
struct Tally {
	...
	table.insert( a, *p );
	// a->second += 1;
	a->second.push_back(v_application_string[counter]);
	counter = counter + 1;
        }
    }
};
...
static void CountOccurrences(int nthreads, int N) {
    ...
    ofstream outputfile("tmp-application_string");  
  ...
  int N = atoi(argv[2]);  
	...
	try {
	  ...
	  for (unsigned int row = 0; row < data.size(); row++) {
	    ...
	    v_application_string.push_back(rec[11].c_str());
