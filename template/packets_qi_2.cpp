...
int main( int argc, char* argv[] ) {
  ...
  for (unsigned int row = 0; row < sdata.size(); row++) {
    ...
    for(size_t c = line_string.find_first_of("\""); c != string::npos; c = c = line_string.find_first_of("\"")){
      line_string.erase(c,1);
    }
    
    string::const_iterator iter = line_string.begin(), end = line_string.end();
