...
int traverse_file(char* filename, char* srchstr, int thread_id) {
	...
	*/     
	...
	pthread_mutex_lock(&tms_string.mutex);

	/*
	  if( timestamp_string[thread_id_local].length() > 0)
	    timestamp_string[thread_id_local].clear();
	*/	

	tms_string.timestamp_string.clear();
	...
	auto str = [=](int x){
	  ...
	  if(ss.str().length() == 1)
	    {
	      tms_string.timestamp_string = tms_string.timestamp_string + "0" + ss.str();
	    }
	  else
	    {
	      tms_string.timestamp_string = tms_string.timestamp_string + ss.str();
	    }
	...
	if(line_counter % 10000 == 0)
	  ...
	  pthread_mutex_unlock(&tms_string.mutex);
