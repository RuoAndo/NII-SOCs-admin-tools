...
typedef struct _ipaddr_string {
    std::string sourceIP_string;
    pthread_mutex_t mutex;    
} ipaddr_string_t;
ipaddr_string_t ipaddr_string;
...
int traverse_file(char* filename, char* srchstr, int thread_id) {
	...
	*/     
	...
	pthread_mutex_lock(&ipaddr_string.mutex);

	ipaddr_string.sourceIP_string.clear();
	
	std::string::iterator first = sourceIP.begin(), last = sourceIP.end();
	...
	auto str = [=](int x){
	  ...
	  if(ipaddr_string.sourceIP_string.empty() == true)
	    ipaddr_string.sourceIP_string = ss.str();
	  else
	    ipaddr_string.sourceIP_string = ipaddr_string.sourceIP_string + "." + ss.str();
	};
	...
	cout << "[" << now_str() << "]:" << ipaddr_string.sourceIP_string << endl;
	
	/*
	if(line_counter % 10000 == 0)
	  {
	    std::cout << "threadID:" << thread_id_local << ":" << filename << ":" << tms_string.timestamp_string << ":" << line_counter << std::endl;
	  }
	    
	  unsigned long long timestamp_ull = stoull(tms_string.timestamp_string);	
	  TimeStamp_msec::accessor a;
	  timestamp_msec.insert(a, timestamp_ull);
	  a->second += 1;
	*/     

	  pthread_mutex_unlock(&ipaddr_string.mutex);
