...
int traverse_file(char* filename, char* filelist_name, int thread_id) {
  ...
  try {
    ...
    for (unsigned int row = 0; row < list_data.size(); row++) {
      ...
      std::cout << "[" << now_str() << "]" << "threadID:" << thread_id << ":" << "(" << list_file << ")" << addr_counter << "(" << list_data.size() << "):" << argIP << "/"
	...
	std::string srcIP = rec2[7];
	std::string destIP = rec2[9];

	/*
	std::cout << "srcIP:" << srcIP << std::endl;
	std::cout << "destIP:" << destIP << std::endl;
	std::cout << "argIP:" << argIP << std::endl;
	*/	
