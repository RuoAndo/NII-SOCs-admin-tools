...
void thread_func1(void *arg) {
      ...
      for (itr = myAddrPair.begin(); itr != myAddrPair.end(); itr++)
	{
	  if(src == itr->first && dst == itr->second) {
	    ...
	    // std::cout << "HIT:" << src << ":" << dst << ":" << alert << std::endl;
	    
	    std::map<int, vector<string>>::iterator it;
	    
	    it = result2.alert_name.find(map_counter);
	    if(it == result2.alert_name.end())
	      {
		// result2.alert_name.insert(std::make_pair(map_counter, alert));
...
int main(int argc, char *argv[])
{
    ...
    for (itr = result.m.begin(); itr != result.m.end(); itr++)
      {
	//std::cout << itr->first << "," << itr->second << "," << alerts[counter] << std::endl;

	counter2 = 0;

	for (itr5 = result2.alert_name.begin(); itr5 != result2.alert_name.end(); itr5++)
	  {
	    string tmp_string;
	    // tmp_string = result2.alert_name[counter2];

	    std::cout << "size:" << result2.alert_name[counter2].size() << std::endl;
	    sort(result2.alert_name[counter2].begin(), result2.alert_name[counter2].end());
	    result2.alert_name[counter2].erase( unique(result2.alert_name[counter2].begin(), result2.alert_name[counter2].end()), result2.alert_name[counter2].end() );

	    std::cout << "size2:" << result2.alert_name[counter2].size() << std::endl;

	    for(int i = 0; i < (int)result2.alert_name[counter2].size(); ++i){
	      std::cout <<  itr->first << "," << itr->second << "," << result2.alert_name[counter2][i] << std::endl;
	    }
