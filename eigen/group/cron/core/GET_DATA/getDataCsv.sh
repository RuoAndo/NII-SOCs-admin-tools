...
OUTPUT_FILE_NAME="./OUTPUT/${TARGET_DATA_KBN}_${FILE_START}-${FILE_END}.csv"
...
############################################
## カラム名取得
COLUMN_NAME_FILE="./OUTPUT/HEADER/${TARGET_DATA_KBN}.csv"
...
if [ "${ZIP_FLG}" = "ON" ];then
    ...
    if [ ${OUTPUT_CSV_PER_1HOURS_FLG} = "ON" ];then
        FILE_START=`date +"%Y%m%d_%H%M" -d "${START_TIME}"`
        FILE_END=`date +"%Y%m%d_%H%M" -d "${END_TIME}"`
        OUTPUT_FILE_NAME="./OUTPUT/${TARGET_DATA_KBN}_${FILE_START}-${FILE_END}"
        ZIP_TARGET_FILE=`echo ${ZIP_TARGET_FILE} | sed -e "s@ @ .\/OUTPUT\/@g" -e "s@^@.\/OUTPUT\/@"`
    fi
