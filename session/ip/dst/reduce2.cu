...
int main( int argc, char* argv[] ) {
  ...
  thrust::host_vector<long> h_vec_2(N);   
	...
	try {
	 ...
	 std::remove("tmp-reduced-dst-final");
	 ofstream outputfile("tmp-reduced-dst-final");
	  ...
	  outputfile << "timestamp, dst, counted" << endl;
	  ...
	  for(long i=0; i <new_size; i++)
	  {
	    ...
	    outputfile << src_string << "," << hvalue_out_2[i] << endl;
