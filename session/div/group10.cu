...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    outputfile << "timestamp, addr" << endl;
    
    for(i=0;i<nData;i++)
    {
       if(h_C[i] == h_B[i])
       {
        out_string = to_string(h_A[i]);

       	// cout << h_A[i] << "," << h_B[i] << "," << h_C[i] << "," << h_D[i] << endl;
	// outputfile << h_A[i] << "," << h_B[i] << "," << h_C[i] << "," << h_D[i] << endl;

        outputfile << out_string.substr( 0, 4 ) << "-" << out_string.substr( 4, 2)
		   << "-" << out_string.substr( 6, 2 ) << " " << out_string.substr( 8, 2 )
		   << ":" << out_string.substr( 10, 2 ) << ":00" << "," << targetIP << endl;
       }
    }
    outputfile.close();
