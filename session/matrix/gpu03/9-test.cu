  ...
  thrust::host_vector<int> h_vec_2(N);   
	...
	try {
	  ...
	  if (!objCsv.getCsv(data)) {
	    ...
	    h_vec_1[row] = std::stoull(timestamp);
	    h_vec_2[row] = std::stoull(rec[1]);
	  ...
	  std::remove("test");
	  ofstream outputfile("test");

	  int counter = 0;
	  for(long i=0; i < value_in.size(); i++)
	    {
		std::string tmpstring = std::to_string(key_in[i]);

		if(std::to_string(key_in[i]).length()  == 14)
		{
		 outputfile << key_in[i] << "," << value_in[i] << endl;
	  	}

		if(counter%1000000==0)
			std::cout << counter << ":done" << endl;
