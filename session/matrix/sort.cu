  ...
  // int counter = 0;
	...
	try {
	  ...
	  if (!objCsv.getCsv(data)) {
	    ...
	    h_vec_2.push_back(std::atol(addr.c_str()));

	    }

	  thrust::device_vector<long> key_in = h_vec_2;
	  thrust::device_vector<long> value_in = h_vec_1; 

	  // thrust::sort_by_(key_in.begin(), key_in.end());
	  // long new_size = new_end.first - dkey_out.begin();

	  thrust::sort_by_key(key_in.begin(), key_in.end(), value_in.begin(),thrust::greater<int>());

	  for(long i=0; i <10; i++)
	    {
		std::cout << key_in[i] << "," << value_in[i] << endl;
	    }
