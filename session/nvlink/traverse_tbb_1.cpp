    ...
    */    
    ...
    for (unsigned int row2 = 0; row2 < session_data.size(); row2++) {
		 ...
		 */
		 ...
		 /*
		 typedef tbb::concurrent_hash_map<long, int> iTbb_Vec_timestamp_inward;
		 static iTbb_Vec_timestamp TbbVec_timestamp_inward;
		 */

		 /*
		 410  CharTable3::accessor a3;
		 411  table3.insert(a3, n);
		 412  a3->second += 1; 
		 */
...
int main(int argc, char* argv[]) {
    ...
    */    
    ...
    for( iTbb_Vec_timestamp_inward::iterator i = TbbVec_timestamp_inward.begin(); i!=TbbVec_timestamp_inward.end(); ++i )
    {
      ...
      salary.insert(std::make_pair(long(i->first),long(i->second)));      
      // counter = counter + 1;
    }

    for(auto itr = salary.begin(); itr != salary.end(); ++itr) {
