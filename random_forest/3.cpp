...
public:
		...
		int maxDepth)
	{
		...
		{
		  // m_trees.emplace_back(DecisionTree(features, answers, minNodeSize, maxDepth, 2, 5));
		  m_trees.emplace_back(DecisionTree(features, answers, minNodeSize, maxDepth, 2, 2));
		  // m_trees.emplace_back(DecisionTree(features, answers, 2, maxDepth, 2, 2));
		}
	}
...
int main(int argc, char *argv[])
{
	...
	for (int k = 0; k < 30; ++k)
	{
		...
		rf->train(trainingFeatures, trainingAnswers, numAdditionalTrees, minNodeSize, maxDepth);
...
//				cout << "Failure! i=" << i << " myAnswer=" << myAnswer << " testAnswer=" << testAnswers[i] << endl;
		...
		// cout << "totalError=" << totalError << endl;
                printf("%d \n", totalError);
		alltotalerror=alltotalerror+totalError;
	}
	

	cout << alltotalerror << endl;
