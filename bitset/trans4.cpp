...
concurrent_vector < string > sourceIP;
  ...
  int N = atoi(argv[2]);  
	...
	try {
	  ...
	  for (unsigned int row = 0; row < data.size(); row++) {
	    ...
	    for (const auto subStr : split_string_2(pair, del)) {
	...
	sv.insert(sv.begin(), sourceIP.begin(), sourceIP.end());
