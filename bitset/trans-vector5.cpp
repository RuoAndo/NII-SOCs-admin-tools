...
int main( int argc, char* argv[] ) {
  ...
  int N = atoi(argv[2]);  
	...
	try {
	  ...
	  for (unsigned int row = 0; row < data.size(); row++) {
	    ...
	    for (const auto subStr : split_string_2(srcPort, del)) {
	      ...
	      std::bitset<8> trans =  std::bitset<8>(port_src);
	      std::string trans_string = trans.to_string();
	    ...
	    for (const auto subStr : split_string_2(destPort, del)) {
	      ...
	      std::bitset<8> trans =  std::bitset<8>(port_dest);
	      std::string trans_string = trans.to_string();
	      destinationPort = destinationPort + trans_string;
	    }
	    
	    string pair_string = srcIP + "," + sourceIP + "," + srcPort + "," + sourcePort + "," + destIP + "," + destinationIP + "," + destPort + "," + destinationPort;
	    
	    string pair_string_2 = sourceIP + sourcePort + destinationIP + destinationPort;
			
	    IPpair.push_back(pair_string_2);	    
	  }
	}
	catch (...) {
	  cout << "EXCEPTION!" << endl;
	...
	for(auto itr = sv.begin(); itr != sv.end(); ++itr) {
	     ...
	     // std::cout << tmp_string << "," << trans_tmp.to_ullong() << endl;
	     // std::cout << tmp_string << "," << trans_tmp.to_ullong() << endl;
	     std::cout << tmp_string << endl;	     
	}
