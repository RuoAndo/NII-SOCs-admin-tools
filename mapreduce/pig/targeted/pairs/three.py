...
while line:
    ...
    try:
        avg = np.c_[a[0][1], a[0][2], a[0][3]]
        ses = np.c_[double(tmp[3]),double(tmp[4]),double(tmp[5])]
        distance = np.linalg.norm(avg-ses)
        #print "cluster 0" + ":" + str(avg) + ":" + str(ses) + ":" + str(distance)
        #print distance
        norms.append(distance)
    
        avg = np.c_[a[1][1], a[1][2],a[1][3]]
        ses = np.c_[double(tmp[3]),double(tmp[4]),double(tmp[5])]
        distance = np.linalg.norm(avg-ses)
        ...
        if(int(tmp[0]) != idx):
            ...
            print str(idx) + "," + tmp[1] + "," + tmp[2] + "," + tmp[3] + "," + tmp[4] + "," + tmp[5].strip()
